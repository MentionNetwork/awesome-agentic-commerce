...
Open standard developed by **Stripe and OpenAI** that powers agentic checkout in ChatGPT. ACP is a server-to-server REST API: the merchant exposes checkout endpoints (create/update/complete checkout session) plus a `delegate_payment` endpoint, and the AI platform calls them to complete a purchase. In 2026 OpenAI refocused ACP-based purchasing on a narrower set of large retailers via ChatGPT Apps. Spec: [agenticcommerce.dev](https://www.agenticcommerce.dev).
...
## UCP vs ACP vs AP2 — which is which?

- **UCP** (Google + Shopify + retailers): full-lifecycle standard from discovery to post-purchase, published as a static manifest. Live in Google AI Mode and Gemini, with the broadest retail backing.
- **ACP** (Stripe + OpenAI): checkout API used by ChatGPT. Server-to-server, transaction-focused; since 2026 refocused on large retailers via ChatGPT Apps.
